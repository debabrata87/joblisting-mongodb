...
public class JobApplicationTestMockMvcWithRepo {
	...
	@Test
	 void filterJobPostTestWithMockRepoDatabaseCalls() throws Exception {
		String searchText = "developer";  // The search text input
        List<Post> mockPosts = List.of(
            new Post("Java Developer", "Some description"),
            new Post("Senior Java Developer", "Another description")
        );

        // Define behavior of repo2 when searchJobPost is called
        Mockito.when(repo2.searchJobPost(searchText)).thenReturn(mockPosts);

        // Act & Assert
        mockMvc.perform(get("/filterjobpost/{searchtext}", searchText))  // Simulate GET request to /jobs/filter/{searchtext}
               .andExpect(status().isOk())  // Check HTTP status is 200 OK
               .andExpect(jsonPath("$[0].profile").value("Java Developer"))  // Check the first post title
               .andExpect(jsonPath("$[1].profile").value("Senior Java Developer"));  // Check the second post title
   }
}
